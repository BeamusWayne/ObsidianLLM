...
- **Clip** — paste a URL, the server fetches and strips it to clean Markdown
- **Ingest** — an LLM reads the article and writes structured wiki pages to `wiki/`
- **Browse** — explore the wiki and the entity graph in the browser

### Highlights

- Multi-backend LLM chain — Ollama (local), Claude API, or any OpenAI-compatible endpoint
- Named preset support: `["preset:qwen-plus", "preset:glm-5", "ollama"]`, auto-fallback on quota errors
- Parallel ingest workers with per-file size routing to different models
